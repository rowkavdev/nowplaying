...
test("connects lazily, publishes, clears and closes", async () => {
  const t=transport(); const client=createDiscordClient({transport:t});
...
test("backs off after connection failure and retries later", async () => {
  ...
  const client=createDiscordClient({transport:t,retryDelayMs:500,now:()=>time});
...
test("disconnects and backs off when publishing fails", async () => {
  ...
  const client=createDiscordClient({transport:t,retryDelayMs:100,now:()=>time});
