...
1. **Connect your media server.** Paste your server's address and sign in. The provider pages show exactly where to find each sign-in: [Plex](Connect-Plex) · [Jellyfin](Connect-Jellyfin) · [Navidrome](Connect-Navidrome) · [Emby](Connect-Emby). Use the connection test on the page; success means nowplaying can see your sessions.
