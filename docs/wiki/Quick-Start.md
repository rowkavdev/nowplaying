...
1. **Connect your media server.** Paste your server's address and sign in. The provider pages show exactly where to find each sign-in: [Plex](Connect-Plex) · [Jellyfin](Connect-Jellyfin) · [Navidrome](Connect-Navidrome) · [Emby](Connect-Emby) · [Spotify](Connect-Spotify). Use the connection test on the page; success means nowplaying can see your sessions. Running more than one server? Use **Add another server** to sign in to each of them (up to eight). Later, **Add or remove servers** on the Settings page reopens setup and nowplaying restarts with the new set. If several are playing at once, the one that started or changed most recently shows; playing beats paused. Watching YouTube in a browser? The [YouTube extension](Connect-YouTube) adds it to your card and Discord.

What each source can show:

| Source | Music | TV episodes | Films |
| --- | --- | --- | --- |
| Plex, Jellyfin, Emby | yes | yes | yes |
| Navidrome | yes | - | - |
| Spotify | yes, and podcasts | - | - |
| YouTube extension | yes (YouTube Music) | - | - |
