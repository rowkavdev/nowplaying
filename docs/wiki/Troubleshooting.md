...
- Open **Settings > Discord** to check whether Discord is enabled and the desktop app is running.
...
## The media server connection failed in Settings

- **Couldn't find a server with that name:** check the address is spelled right, or use the server's IP address instead.
- **Certificate isn't trusted:** the server's security certificate is self-signed or out of date. Give the server a certificate Windows trusts, or use its local `http://` address. NowPlaying does not skip certificate checks.
