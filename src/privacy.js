...
export function validatePrivacyPolicy(input = {}) {
  ...
  for (const key of Object.keys(input)) {
    if (!allowed.has(key)) throw new TypeError(`privacy.${key}: unknown setting`);
  }
  if (input.mode !== undefined && !PRIVACY_MODES.has(input.mode)) {
  ...
  for (const key of ["hideUsers", "hideProviders", "hideArtwork", "redactTitles", "hideProgress"]) {
    if (input[key] !== undefined && typeof input[key] !== "boolean") {
      throw new TypeError(`privacy.${key}: expected a boolean`);
    }
  }
  if (input.titleReplacement !== undefined && (typeof input.titleReplacement !== "string" || input.titleReplacement.length > 128)) {
    throw new TypeError("privacy.titleReplacement: expected a string up to 128 characters");
  }
  if (input.suppressMediaKinds !== undefined) {
    if (!Array.isArray(input.suppressMediaKinds)) {
      throw new TypeError("privacy.suppressMediaKinds: expected an array");
    }
    for (const kind of input.suppressMediaKinds) {
      if (!MEDIA_KINDS.has(kind)) {
        throw new TypeError(`privacy.suppressMediaKinds: unknown media kind ${String(kind)}`);
      }
    }
  }
  return input;
...
export function createPrivacyPolicy(input = {}) {
  ...
  return Object.freeze({
    ...privacyDefaults,
    ...input,
    suppressMediaKinds: Object.freeze([...(input.suppressMediaKinds ?? privacyDefaults.suppressMediaKinds)]),
  });
}
...
export function applyPrivacy(presence, input = {}) {
  ...
  if (policy.mode === "private" || policy.suppressMediaKinds.includes(presence.kind)) {
    return createPresence({ state: "idle", updatedAt: presence.updatedAt });
  }
